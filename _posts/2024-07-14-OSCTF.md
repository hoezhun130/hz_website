---
layout: "post"
title: "OS CTF"
categories: [OSCTF]
tags: "Base64 Hex CaesarCipher RSA"
image: /assets/CTF/OSCTF/logo.png
---

## Crypto/Cipher Conundrum

**Solution**

In this challenge, multiple steps are required to decrypt the encrypted text.

- Base64:

![light mode only](/assets/CTF/OSCTF/Crypto/base64.png){: .light .w-75 .shadow .rounded-10 w='1212' h='668' }
![dark mode only](/assets/CTF/OSCTF/Crypto/base64.png){: .dark .w-75 .shadow .rounded-10 w='1212' h='668' }

- Hex:

![light mode only](/assets/CTF/OSCTF/Crypto/hex.png){: .light .w-75 .shadow .rounded-10 w='1212' h='668' }
![dark mode only](/assets/CTF/OSCTF/Crypto/hex.png){: .dark .w-75 .shadow .rounded-10 w='1212' h='668' }

- Caesar cipher:

![light mode only](/assets/CTF/OSCTF/Crypto/caesar.png){: .light .w-75 .shadow .rounded-10 w='1212' h='668' }
![dark mode only](/assets/CTF/OSCTF/Crypto/caesar.png){: .dark .w-75 .shadow .rounded-10 w='1212' h='668' }

We successfully decrypted it to reveal the flag.

![light mode only](/assets/CTF/OSCTF/Crypto/flag.png){: .light .w-75 .shadow .rounded-10 w='1212' h='668' }
![dark mode only](/assets/CTF/OSCTF/Crypto/flag.png){: .dark .w-75 .shadow .rounded-10 w='1212' h='668' }

**Flag:**

`OSCTF{5o_M3nY_C1ph3Rsssss}`

## Crypto/Couple Primes

**Challenge Description:**

We are given a RSA encryption setup and the parameters `n` and `c`. 

```python
from Crypto.Util.number import *
from sympy import nextprime

flag = b'REDACTED'

p = getPrime(1024)
q = nextprime(p)
e = 65537

n = p * q
c = pow(bytes_to_long(flag), e, n)

print(f"n = {n}")
print(f"c = {c}")
```

**Solution:**

To decrypt the ciphertext `c` to retrieve the flag, we need to factorize `n` to find the primes `p` and `q`. Given that `q` is the next prime after `p`, we can use this property to find the primes.

Here is the complete solution:

```python
from Crypto.Util.number import long_to_bytes
from sympy import nextprime, isprime
from math import isqrt

n = 
c = 
e = 65537

approx_p = isqrt(n)

while not isprime(approx_p):
    approx_p -= 1

p = approx_p
q = nextprime(p)

assert p * q == n

phi = (p - 1) * (q - 1)
d = pow(e, -1, phi)

m = pow(c, d, n)
flag = long_to_bytes(m)

print(flag.decode())
```

**Flag:**

OSCTF{m4y_7h3_pR1m3_10v3_34cH_07h3r?}